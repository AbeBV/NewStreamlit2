...
@st.cache
def get_dataCSV(filename, indexCol = True,header = True):
	if indexCol == True:
		...
		return df
	#if header == False:
		#df = pd.read_csv(filename, index_col = [0], header = False)
		#return df
...
with features:
	...
	dfOutlierPerMonth_Filtered.reset_index(drop = True, inplace = True)
	st.write(dfOutlierPerMonth_Filtered.style.set_precision(4))
